function set_menu_active(){
	$("#active-menu-id-wx-pre-orders").addClass("active").siblings().removeClass("active");
}
set_menu_active();
...
function getStoreOrderLists() {
    ...
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getStoreWxPreOrderList",
        dataType: "json",
        success: function(e) {
            if (e) {
            	//订单列表          	
...
function search_by_order_type(search_type){
	...
	if(search_type == 'search_all'){
		$order_from = 0;
	}else if(search_type == 'state_new'){
		$state_type = 'state_new';
	}else if(search_type == 'state_confirm'){
		$state_type = 'state_confirm';
	}else if(search_type == 'state_success'){
		$state_type = 'state_success';
	}else if(search_type == 'state_cancel'){
...
function reset_search_by_key(){
	$("#input-order-id").addClass("ng-hide");
	$("#input-order-phone").addClass("ng-hide");
	$("#input-order-name").addClass("ng-hide");
...
function search_by_key(key){
	...
	}else if(key == 'key-order-phone'){
		$("#input-order-phone").removeClass("ng-hide");
	}else if(key == 'key-order-name'){
		$("#input-order-name").removeClass("ng-hide");
...
function search(){
	...
	$order_phone = $("#inputed-order-phone").val();
	$order_name = $("#inputed-order-name").val();
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getStoreWxPreOrderList",
        dataType: "json",
        data: {"order_sn":$order_id,"order_phone":$order_phone,"order_name":$order_name},
        success: function(e) {
            if (e) {
            	//订单列表          	
            	var html = template.render('order_list', e.data);
...
function searchByDeliveryTime(){
	...
	$.ajax({
        ...
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getStoreWxPreOrderList",
        dataType: "json",
        data: {"reserv_date":$delivery_time},
        success: function(e) {
            if (e) {
            	//订单列表          	
            	var html = template.render('order_list', e.data);
