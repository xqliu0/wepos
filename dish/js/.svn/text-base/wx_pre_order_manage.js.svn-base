...
function cancel_order(){
	...
	$order_state = $('#oper-table-order-state').val();
	if($order_state == 0 || $order_state == 30){
		return;
	}
...
function finish_order(){
	...
	$("#finish-order-panel").removeClass("ng-hide");
...
function confirm_order(){
	...
	$order_state = $('#oper-table-order-state').val();
	if($order_state == 0 || $order_state == 20 || $order_state == 30){
		return;
	}
	$("#confirm-order-panel").removeClass("ng-hide");
...
function cancel_order_submit(){
	...
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=cancelWxPreOrder",
        dataType: "json",
...
function finish_order_submit(){
	...
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=finishWxPreOrder",
        dataType: "json",
        data: {"store_id":store_id,"table_id":table_id,
        	"order_id":order_id},
        success: function(e) {
...
function confirm_order_submit(){
	...
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=confirmWxPreOrder",
        dataType: "json",
        data: {"store_id":store_id,"table_id":table_id,
        	"order_id":order_id},
        success: function(e) {
...
function top_level_choose_table(store_id, table_id,order_id, order_state,order_from,buyer_name) {
	...
	$('#oper-table-order-state').val(order_state);
...
function set_oper_button_state_4_orders(order_from,order_state){	
	...
	set_oper_state_wx_pre_order(order_state);
}
...
function set_oper_state_wx_pre_order(order_state){
	//已取消
	if(order_state == 0){
		//已取消
		...
		//取消订单
		$('#ordered-table-action-cancle-order').removeClass('ng-hide');
		$('#ordered-table-action-cancle-order').removeClass('disabled');

		$('#ordered-table-action-finish-order').removeClass('ng-hide');
		$('#ordered-table-action-finish-order').removeClass('disabled');
		...
		$('#ordered-table-action-confirm-order').addClass('disabled');
		...
		$('#ordered-table-action-confirm-order').addClass('disabled');
		$('#ordered-table-action-cancle-order').addClass('disabled');
		$('#ordered-table-action-finish-order').addClass('ng-hide');
...
function getPaidStoreTableOrderInfo(order_id, store_id, table_id) {
    ...
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getStoreWxPreOrderInfo",
        dataType: "json",
        data: {"order_id":order_id,"store_id":store_id,"table_id":table_id},
        success: function(e) {
            if (e) {
            	
            	var html = template.render('store_table_order_info', e.data.order_list);
				$("#store_table_order_info_zone").html(html); 
				

        		//纪录对应的订单id号信息
            $("#oper-table-order-id").val(e.data.order_list.pre_order_sn);
...
function click_mask() {
	...
	$('#finish-order-panel').addClass('ng-hide');
	$('#confirm-order-panel').addClass('ng-hide');
...
function go_checkout(){
	...
	+ "&tid=" + table_id + "&oid=" + order_id + "&ptid=" + parent_table_id;
}
