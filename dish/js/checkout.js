            ...
            if (e) {
				...
				if(e.data.order_info.wepos_pay_state > 0){
					var html = template.render('paid_list', {'paidArray':e.data.order_info.wepos_pay_list});
			  		...
			  		$('#normal_pay_mode_confirm').addClass("disabled");
				} else {
					$('#normal_pay_mode_confirm').removeClass("disabled");
				}
				payMode.setPayMode(e.data.order_info.payment_mode);
            }
        },
        error: function() {
...
function select_pay_method_name(method){
	...
	//普通支付
	if (payMode.getMode() == 0) {
		executePayment(method, remain_amount);
		return;
	}
...
function executePayment(method, amount) {
	...
	payDetailArray.splice(0, payDetailArray.length);
	...
	$.ajax({
        ...
        dataType: "json",
        data: {"order_id":oid, "store_id":sid, "table_id":tid, "payDetailArray":payDetailArray, "returnResult": "ture"},
        success: function(e) {
            if (e.error == "") {
            	change_pay_item_to_paid(e.data.payDetailArray[0].wepos_pay_id, method, amount);
            }
        },
        error: function() {
            $("#tip").show().html("服务器异常，请稍后再试"),
...
//        	spinner.stop();
    		if(e.result == true){
    			...
    			$('#wx-sm-pay-result').text('微信扫码支付结果：'+e.error);
    			wx_sm_pay_spinner.stop();
    		}
        },
        error: function() {
...
function add_money(money){
	var paid_money = $('#cash-pay-panel-paid-amount').val();
	paid_money = parseFloat(paid_money) + parseFloat(money);
	paid_money = p2f(paid_money);
	
	$('#cash-pay-panel-paid-amount-input').val(paid_money);
	$('#cash-pay-panel-paid-amount').val(paid_money);
...
function refresh_wxscan_pay(){
	...
	var checkout_table_name = $('#checkout_table_name').text();
	var good_name = wepos_store_name+'-'+checkout_table_name;
	var pay_amount = $('#wx-pay-panel-total-amount').val();
...
function confirm_pay(){
	...
	var paid_money = $('#cash-pay-panel-paid-amount-input').val();
	var total_money = $('#cash-pay-panel-total-amount').val();
	if (p2f(paid_money) < parseFloat(total_money)) {
		return;
	}
	var exchange_money = $('#cash-pay-panel-paid-amount-exchange').val();
	var tid = $('#selected-table-id').val();
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=confirmPay",
        dataType: "json",
        data: {"order_id":order_id,"table_id":tid, "wepos_pay_id":wepos_pay_id,"pay_code":pay_code, "paid_money":paid_money, "exchange_money":exchange_money},
        success: function(e) {
            if (e.error == "") {
            	//alert(e.data.all_paid);
            	if(e.data.all_paid == true){
            		var sid = $('#selected-store-id').val();
            		var tid = $('#selected-table-id').val();
            		var ptid = $('#selected-parent-table-id').val();
            		var oid = $('#selected-order-id').val();
...
function clear_pay_items(){
	$('#confirm-clear-pay-panel').removeClass("ng-hide");
}
...
function clear_pay(){
	...
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=clearPay",
        dataType: "json",
...
function click_mask(){
	...
	$('#pay-money-amount-panel').addClass("ng-hide");
	$('#cash-pay-panel').addClass("ng-hide");
	$('#confirm-pay-panel').addClass("ng-hide");
	$('#confirm-clear-pay-panel').addClass("ng-hide");
	$('#confirm-abort-pay-panel').addClass("ng-hide");
	
	$('#note-panel').addClass("ng-hide");
	$('#cancel-privilege-confirm-panel').addClass("ng-hide");
	$('#wx-pay-panel').addClass("ng-hide");
	$('#wx-sm-pay-panel').addClass("ng-hide");
	$('#moling-change-confirm-panel').addClass("ng-hide");
	$('#discount-confirm-panel').addClass("ng-hide");
	$('#free-charge-confirm-panel').addClass("ng-hide");

	
	//关闭扫描时钟
	clearInterval(scan_intervalId);
	clearInterval(wxpay_result_query_intervalId);
}
    
function reinput_paid_amount(id) {
	re_input(id);
	$('#cash-pay-panel-paid-amount-exchange').text(0);
	$('#cash-pay-panel-paid-amount-exchange').val(0);
	$('#cash-pay-panel-paid-amount').val(0);
	$('#cash-pay-panel-confirm').addClass("disabled");
}

function re_input(id){
	$('#'+id).val("");
	$('#'+id).text("");
}
...
(function(owner) {
	var payMode = 0; // 0-普通模式，1-组合模式  

	owner.setPayMode = function(mode) {
		payMode = mode;
		if (mode == 0) {
			$('#pay_list_zone').addClass('ng-hide');
			$('#submit_zone').addClass('ng-hide');
			
			$('#normal_pay_mode_confirm').addClass('active');
			$('#compose_pay_mode_confirm').removeClass('active');
		} else {
			$('#pay_list_zone').removeClass('ng-hide');
			$('#submit_zone').removeClass('ng-hide');

			$('#normal_pay_mode_confirm').removeClass('active');
			$('#compose_pay_mode_confirm').addClass('active');
		}
	}
	
	owner.switch = function(mode) {
		if (typeof(payDetailArray) != 'undefined') {
			payDetailArray.splice(0, payDetailArray.length);
		}
		var order_id = $('#selected-order-id').val();
		$.ajax({
        type: "GET",
	        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=updatePaymentMode",
	        dataType: "json",
	        data: {"payment_mode":mode, "order_id":order_id},
	        success: function(e) {
	            if (e) {
	            	owner.setPayMode(mode);
	            }
	        },
	        error: function() {
	            $("#tip").show().html("服务器异常，请稍后再试"),
