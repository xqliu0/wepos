...
function refresh_store_setting(setting) {
	if (setting['moling_type'] == '0') {
...
function isOnShift() {
	var is_on_shift = storeWeposSettingArray['is_on_shift'];
	return is_on_shift == 1;
}

function open_shift(){
	if($("#open_shift").hasClass('disabled')){
		return;
	}
	var shift_state = storeWeposSettingArray['shift_state'];
	var is_on_shift = storeWeposSettingArray['is_on_shift'];
        ...
        success: function(e) {
            if (e) {
            	if(e.error==''){
            		...
            		refresh_module();
            		refresh_store_setting(e.data.store_setting);
            	}else{
            		Zepto.sDialog({
    	                skin:"red",
    	                content: "获取用户信息失败，请重新登录",
...
function change_active_menu(menu) {
	if(menu == "home"){
		$('#confirm-abort-pay-panel').removeClass("ng-hide");
		$('#abort-pay-confirm').click(function() {
...
function confirm_sign_out(){
	$('#sign-out-confirm-panel').removeClass('ng-hide');
	$('#sign-out-confirm-action-yes').removeClass('ng-hide');
	$('#sign-out-confirm-action-no').removeClass('ng-hide');
}

function click_sign_out_mask(){
	$('#sign-out-confirm-panel').addClass('ng-hide');
	$('#sign-out-confirm-action-yes').addClass('ng-hide');
	$('#sign-out-confirm-action-no').addClass('ng-hide');
}
