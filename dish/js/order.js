function getStoreGoodsClassInfo() {
    ...
    $.ajax({
        ...
        dataType: "json",
        data: {"table_class_id":0,"store_id":sid,"table_id":tid},
        success: function(e) {
            if (e) {
    ...
    $.ajax({
        ...
        success: function(e) {
            if (e) {
            	...
            	$('#table-cart-table-name').text(e.data.table_info.stc_parent_name + '-' + e.data.table_info.stc_name);
        ...
        success: function(e) {     	 
        	if(e.error == ""){
        		if(e.data.cart_id != true){
            		...
            		selectedGoodsArray.push({'good_id':good_id,'good_name':good_name,'good_price':good_price,'good_num':1,'good_amount':parseFloat(good_price),'good_state':good_state,'cart_id':cart_id,'note':'',
            			'gift':'0'});	
...
function click_mask() {
	...
	$('#create-order-confirm-panel').addClass('ng-hide');
	$('#store_dish_note_select_panel').addClass('ng-hide');
...
function print_order() {
	...
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos_printer&op=getPrintInfo&type=1&storeId=" + store_id + "&orderId=" + order_id,
		success: function(data) {
			...
			if (printInfo.state == 'success') {
				...
				var printList = printInfo.printer_info;
				for (var prt in printList) {
					var result = LxDialog.print(printList[prt].printer, printList[prt].content);
...
function add_selected_good_to_order() {
	...
	var store_id = $('#selected-store-id').val();
	var table_id = $('#selected-table-id').val();
	var table_name = $('#table-cart-table-name').text();
	...
	$.ajax({
        ...
        dataType: "json",
        data: {"store_id":store_id, "table_id":table_id, "table_name":table_name,"selectedGoods":selectedGoodsArray},
        success: function(e) {
            if (e) {
            	if (e.error=='') {
            	   //落单成功
...
function go_table(){
	...
	window.location.href= "index.html?sid=" + sid + "&tid=" + tid + "&ptid=" + ptid;
}

function open_start_print_panel() {
	//展示开台操作div
	$('#create-order-confirm-panel').removeClass('ng-hide');
	$('#create-order-confirm-panel').attr("wp-confirm-info","落单成功，您取消打印该订单吗？");
	$('#create-order-confirm-panel-content').text("落单成功，您取消打印该订单吗？");
	
	$('#create-order-confirm-action-yes').removeClass('ng-hide');
	$('#create-order-confirm-action-no').removeClass('ng-hide');	
}
