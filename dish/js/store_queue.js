...
function getStoreQueueListInfo() {
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getQueueListV2",
        dataType: "json",
        data: {"queue_id":0},
        success: function(e) {
            if (e.error == "" && e.data.length>0) { 

            	$("#create-queue-btn").removeClass("disabled");
            	
              	var html = template.render('store_queue_list', e.data);
				$("#store_queue_list_zone").html(html); 
				getQueueInfoV2(e.data.remote_queue_list.queue_list[0].queue_t_type, e.data.queue_token, e.data.remote_queue_list.store_id);
				
				html = template.render('oper_store_queue_list', e.data);
				$("#oper_store_queue_list_zone").html(html);
				
				$("#join_queue_token").val(e.data.queue_token);
				$("#join_queue_store_id").val(e.data.remote_queue_list.store_id);
				
				$("#join_queue_id").val(e.data.remote_queue_list.queue_list[0].queue_t_type);
				//默认给了一个值，和页面展示相对应
				$("#join_queue_cus_num").val(2); 
            }else{
            	$("#create-queue-btn").addClass("disabled");
            }
        },
        error: function() {
...
function getQueueInfoV2(queue_type,queue_token,remote_store_id) {
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getQueueDetailV2",
        dataType: "json",
        data: {"queue_type":queue_type,"queue_token":queue_token,"remote_store_id":remote_store_id},
        success: function(e) {
            if (e.error == "") {
				...
				$("#queue_"+queue_type).siblings().removeClass("active");
				$("#queue_"+queue_type).addClass("active");
...
function get_history(queue_type,queue_token,remote_store_id){
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getQueueHistoryV2",
        dataType: "json",
        data: {"queue_type":queue_type,"queue_token":queue_token,"remote_store_id":remote_store_id},
        success: function(e) {
            if (e.error == "") {
...
function join_queue_success(){
    $("#join-queue-success-confirm-panel").removeClass("ng-hide");
}

function confirmClearQueue(){
	
	var queue_t_token = $("#join_queue_token").val();
	var queue_t_store_id = $("#join_queue_store_id").val();
	
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=clearQueueV2&queue_token="+queue_t_token+"&queue_store_id="+queue_t_store_id,
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
            	location.reload();
...
function confirmUpdateQueue(){
	...
	var queue_token = $("#selected_queue_token").val();
	var action = $("#selected_queue_action").val();
	
	$.ajax({
        type: "post",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=updateQueueStateV2",
        data: {"queueid":queue_id,"action":action, "queue_token":queue_token},
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
                click_mask();
            	get_history('history', $("#join_queue_token").val(), $("#join_queue_store_id").val());	
            }
        },
        error: function() {
            $("#tip").show().html("服务器异常，请稍后再试"),
...
function updateQueue(queue_id, queue_code,queue_token, action){
	$("#selected_queue_id").val(queue_id);
	$("#selected_queue_code").val(queue_code);
	$("#selected_queue_token").val(queue_token);
...
function click_mask(){
    ...
    $("#join-queue-success-confirm-panel").addClass("ng-hide");
}
...
function join_queue(){
	...
	if(!(/^1[34578]\d{9}$/.test(customer_phone))){ 
        Zepto.sDialog({
             skin:"red",
	...
	$.ajax({
        type: "post",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=joinQueueV2",
        data: {"queue_t_id":queue_t_id,"queue_store_id":queue_t_store_id,"queue_t_token":queue_t_token,"customer_number":customer_number,"customer_phone":customer_phone},
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
            	join_queue_success();     
                getQueueInfoV2(queue_t_id, $("#join_queue_token").val(), queue_t_store_id);      	
            	reprint(queue_t_store_id,queue_t_token,e.data.queue_number,e.data.wait_num,1);
            }
        },
        error: function() {
            $("#tip").show().html("服务器异常，请稍后再试"),
...
function reprint(store_id,token,queue_number,wait_num,ajax){
	//调用打印信息获取接口
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos_printer&op=getPrintInfo&type=4&storeId=" + store_id + "&token=" + token + "&queue_number=" + queue_number + "&wait_num=" + wait_num,
        //data: {"store_id":store_id},
		success: function(data) {
			var printInfo = eval('('+data+')');
			//alert(printInfo.state);
			if (printInfo.state == 'success' && typeof(LxDialog) != 'undefined') {
