...
function getStoreTableClassInfo() {
    $.ajax({
        type: "GET",
        ...
        success: function(e) {
            if (e) {     
            	var html = template.render('store_table_class', e.data);
				$("#store_table_class_zone").html(html); 
				var ptid = GetQueryString("ptid");
				if(ptid == null || ptid ==""){
//					getStoreTableInfo(e.data.table_class[0].stc_id);
					show_all();
				}else{
					getStoreTableInfo(ptid);
				}
				// alert(wepos.getPayInfo());
            }
        },
        error: function() {
            Zepto.sDialog({
...
function get_filtered_tables(state, excludes) {
    $.ajax({
        type: "GET",
        ...
        success: function(e) {
            if (e) {     
				...
				for (var i=0; i < storeTableArray.length; i++) {
					$("#selected-parent-table-class-name-"+storeTableArray[i].stc_id).val(storeTableArray[i].stc_parent_name);
			    }
...
function toggle_action(action) {
	...
	$('#selected-selected-good-name').val('');
	$('#selected-selected-good-price').val('');
	$('#selected-selected-good-num').val('');
	if (good_id != '') {
		$('#selected_line_'+good_id).removeClass("active");
	...
	if(action == 'change_table'){
		...
		$("#tableChooseCell").removeClass("ng-hide");
		...
		$("#navigation-toolbar-note").show(3000);
		...
		//展示空闲的桌台
		get_filtered_tables('ordered', $('#oper-table-table-id').val());
		//展示提示信息
		show_action_note(action);
...
function backToTableDesktop() {
	//恢复桌面展示
	$("#returnProductChooseCell").addClass("ng-hide");
	$("#productChooseCell").addClass("ng-hide");
	$("#tableChooseCell").removeClass("ng-hide");
	$("#table_change_note").addClass("ng-hide");
	$("#table_change_note").siblings().removeClass("ng-hide");
	$("#table_merge_note").addClass("ng-hide");
	$("#navigation-toolbar-note").hide();

	//清空操作
	$("#oper-table-oper-type").val("");
	$("#ordered-table-action-confirm-order").siblings().removeClass("active");
	$("#ordered-table-action-confirm-order").addClass("active");
...
function choose_table(store_id, parent_table_id, table_id, table_name, table_state,seat_num,order_id) {
	if (!isOnShift()) {
		return;
	}
	var action = $('#oper-table-oper-type').val();
	if(action == ""){
		top_level_choose_table(store_id, parent_table_id, table_id, table_name, table_state,seat_num,order_id);
	}else{
...
//首次选择餐桌的操作
function top_level_choose_table(store_id, parent_table_id, table_id, table_name, table_state,seat_num,order_id) {
   ...
   //展示餐桌信息和已点菜品列表区域
	...
	var parent_class_name = $('#selected-parent-table-class-name-'+table_id).val();
	$('#table-item-zone-table-name').text(parent_class_name+ '-' + table_name);
...
function change_order_good_price_free(){
	//设置action
	$('#oper-table-oper-type').val("change_free");

	var good_id = $('#selected-selected-good-id').val();
	if(good_id == "" || good_id == "undefined") return;
	
	var good_name = $('#selected-selected-good-name').val();
	var good_price = $('#selected-selected-good-price').val();
	var good_num = $('#selected-selected-good-num').val();
	
	$('#confirm-panel').removeClass('ng-hide');
	var note = "确定要赠送菜品：" + good_name + "（原价" + good_price + "元，共" + (good_num * good_price).toFixed(2) + "元）吗？";
	$('#confirm-panel').attr("wp-confirm-info",note);
	$('#confirm-panel-content').text(note);
        ...
        success: function(e) {
            if (e) {
            	if (e.error=='') {
                	...
                	//动态刷新桌台显示的价格
                	$("#table_item_price_" + table_id).text("￥ " + e.data.order_amount);
	                ...
	                for (var i=0; i < length; i++) {
						inOrderedGoodsArray.push({'good_id':e.data.ordered_goods_list[i].goods_id,
							...
							'good_new_price':e.data.ordered_goods_list[i].goods_new_price,
							'good_num':e.data.ordered_goods_list[i].goods_num,
							'has_ordered_good_num':e.data.ordered_goods_list[i].goods_num,
							'note':e.data.ordered_goods_list[i].note,
							'gift':e.data.ordered_goods_list[i].gift,
...
function open_table_panel() {
	...
	$('#guest_num_select_page').val('1');
	guest_num_select_pre_page();
	set_guest_num(4);
}
...
function guest_num_select_pre_page(){
	...
	$("#guest_num_val_1").text(alignCenter((page_val-1)*5+1));
	if($("#guest_num_val_1").text()==seat_num){
		$('#guest_num_1').addClass('active').siblings().removeClass("active");
	}else{
		$('#guest_num_1').removeClass("active");
	...
	$("#guest_num_val_2").text(alignCenter((page_val-1)*5+2));
	if($("#guest_num_val_2").text()==seat_num){
		$('#guest_num_2').addClass('active').siblings().removeClass("active");
	}else{
		$('#guest_num_2').removeClass("active");
	...
	$("#guest_num_val_3").text(alignCenter((page_val-1)*5+3));
	if($("#guest_num_val_3").text()==seat_num){
		$('#guest_num_3').addClass('active').siblings().removeClass("active");
	}else{
		$('#guest_num_3').removeClass("active");
	...
	$("#guest_num_val_4").text(alignCenter((page_val-1)*5+4));
	if($("#guest_num_val_4").text()==seat_num){
		$('#guest_num_4').addClass('active').siblings().removeClass("active");
	}else{
		$('#guest_num_4').removeClass("active");
	...
	$("#guest_num_val_5").text(alignCenter((page_val-1)*5+5));
	if($("#guest_num_val_5").text()==seat_num){
		$('#guest_num_5').addClass('active').siblings().removeClass("active");
	}else{
		$('#guest_num_5').removeClass("active");
        ...
        success: function(e) {
            if (e) {
            	if(e.error==''){
            		...
            		var parentTableName = $('#selected-parent-table-class-name-' + $('#oper-table-table-id').val()).val();
            		$('#table-item-zone-table-name').text(parentTableName + '-' + $('#oper-table-table-name').val());
...
function print_order_detail(){
	...
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos_printer&op=getPrintInfo&type=3&storeId=" + store_id + "&orderId=" + order_id,
		success: function(data) {
			var printInfo = eval('('+data+')');
			//alert(printInfo.state);
			if (printInfo.state == 'success') {
				//alert(printInfo.printer_name);
				//alert(printInfo.print_data);
				var result = LxDialog.print(printInfo.printer_name, printInfo.print_data);
				//alert(result.resultCode);
				if (result.resultCode != 0) {
					alert("打印异常，异常码：" + result.resultCode + "，异常信息:" + result.resultInfo);
				}
			} else {
				//TODO: 打印信息异常处理
			}
...
function choose_reprint_targets(){
	...
	$.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos_printer&op=getPrintInfo&type=2&storeId=" + store_id + "&orderId=" + order_id + "&goodsId=" + good_id,
...
function go_checkout(){
	...
	+ "&tid=" + table_id + "&oid=" + order_id + "&ptid=" + parent_table_id + "&type=table";
}
