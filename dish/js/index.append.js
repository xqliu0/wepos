...
function do_append_good(good_id) {
    ...
    for (var i=0; i < length; i++) {
    	if(appendedGoodsArray[i]['good_id'] == good_id){
    		has_inserted = 1;
    		//修改已经选中的菜品中的菜品的数量    		
    		var good_price = parseFloat(appendedGoodsArray[i]['good_amount']) + parseFloat(good_new_price);
    ...
    for (var i=0; i < length; i++) {
    	if(appendedGoodsArray[i]['good_id'] == good_id){
    		if(appendedGoodsArray[i]['good_num'] > 1){
    			...
    			appendedGoodsArray[i]['good_amount'] = p2f(appendedGoodsArray[i]['good_amount'] - good_price);
	    		break;
