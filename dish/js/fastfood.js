function set_menu_active(){
	$("#active-menu-id-fastfood").addClass("active").siblings().removeClass("active");
}
set_menu_active();


function getStoreFastFoodNumber() {

    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=getFastFoodNumber",
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
                $('#food_number').val(e.data.fastfood_number); 
                $("#food_number_manul").text(e.data.fastfood_number);
            }
        },
        error: function() {
...
function getStoreGoodsClassInfo() {
	...
	$('#selected-table-id').val('10001');//快餐订单
...
function filter_goods(){
	var filter_var =  $("#filter-var-fastfood").val();
	if(filter_var != ""){
		filteredStoreGoodArray = [];
            ...
            if (e) {
            	...
            	if(length > 0){
                	...
                	for (var i=0; i < length; i++) {
                		...
                		selected_total_price = parseFloat(selected_total_price) + parseFloat(e.data.cart_info[i]['goods_price'])*parseFloat(e.data.cart_info[i]['goods_num']);
                	}
        ...
        success: function(e) {     	 
        	if(e.error == ""){
        		if(e.data.cart_id != true){
                	...
                	selected_total_price = parseFloat(selected_total_price) + parseFloat(good_price);
                	//selected_total_price = p2f(selected_total_price);
	...
	$.ajax({
        ...
        success: function(e) {
        	if(e.error == ""){
	    		...
	    		selectedGoodsArray[index]['good_amount'] += parseFloat(good_price);
	    		selected_total_price = parseFloat(selected_total_price)+parseFloat(good_price);
	    		//更新选中的菜品列表区域
	    		refreshSelectedGoods(good_id);
...
function add_good(good_id) {
    ...
    //第一次选择菜品自动更新牌号
    if ($('#order-good-delivery-info').hasClass('ng-hide')) {
        refreshStoreFastFoodNumber();
    }
...
function refreshStoreFastFoodNumber() {
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=refreshFastFoodNumber",
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
                $("#food_number_manul").text(e.data.fastfood_number);
                $("#food_number").val(e.data.fastfood_number);
                $('#fastfood-number-panel').addClass('ng-hide');
            }
        },
        error: function() {
            $("#tip").show().html("服务器异常，请稍后再试"),
...
function add_fastfood_number(){
	$("#fastfood-number-panel").removeClass('ng-hide');
}
...
function input_fastfood_number(){
	var fastfood_number = $("#fastfood-number-panel-input").val();
	if(fastfood_number == "" || parseFloat(fastfood_number) <= 0){
		$("#food_number_confirm").addClass('disabled');
	}else{
		$("#food_number_confirm").removeClass('disabled');
	}
}

function food_number_confirm(){
	var fastfood_number = $("#fastfood-number-panel-input").val();
	$("#food_number_manul").text(fastfood_number);
	$("#food_number").val(fastfood_number);
	
	$('#fastfood-number-panel').addClass('ng-hide');
}

function setStoreFastFoodNumber() {
    var fastfood_number = $("#fastfood-number-panel-input").val();
    var reg = new RegExp("^[0-9]*$");  
    if(!reg.test(fastfood_number)){  
        Zepto.sDialog({
                skin:"red",
                content: "请输入数字",
                okBtn:false,
    ...
    $.ajax({
        type: "GET",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=setFastFoodNumber&fastfood_number="+fastfood_number,
        dataType: "json",
        success: function(e) {
            if (e.error == "") {
                $("#food_number_manul").text(fastfood_number);
...
function set_delivery_info(){
	...
	var user_time = $("#delivery-address-time").val();
...
function refreshSelectedGoods(good_id){
	...
	$('#selected_goods_total_price').text('合计: ￥'+p2f(selected_total_price));
...
function note_good(good_id) {
  ...
  for (var i=0; i < length; i++) {
	    	if(selectedGoodsArray[i]['good_id'] == good_id){
	    		selectedGoodsArray[i]['note'] = cart_good_note;
	    		$('#store_dish_note_select_panel').addClass('ng-hide');
	    		//更新添加的菜品列表区域
	    		refreshSelectedGoods(good_id);
	    		break;
...
function add_selected_good_to_order() {
	...
	var fast_food_note = $('#fast-food-note-input').val();
	var food_number = $('#food_number').val();
	
	//落单
	$.ajax({
        type: "POST",
        url:"http://m.xiaoweicanting.com/ttmr/shop/index.php?act=store_dish_wepos&op=createFastFoodOrder",
        dataType: "json",
        data: {"store_id":store_id, "table_id":table_id,"selectedGoods":selectedGoodsArray,
        	"food_number":food_number,
        	"fast_food_note":fast_food_note},
        success: function(e) {
            if (e) {
            	if(e.error==''){
            	//落单成功
//            		alert(e.data.order_id);
            		window.location.href = 'http://m.xiaoweicanting.com/ttmr/shop/wepos/dish/checkout.html?sid='+store_id+'&tid='+table_id+'&oid='+e.data.order_id+"&type=fast";
            	}
