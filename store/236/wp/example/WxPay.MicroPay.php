...
//require_once (BASE_PATH.'/wepos/14/wp/lib/WxPay.Api.php');
...
class MicroPay
{
    /**
     *
     * 提交刷卡支付分解接口1，并且确认结果，接口比较慢
     * @param WxPayMicroPay $microPayInput
     * @throws WxpayException
     * @return 返回查询接口的结果
     */
    public function payStep1($microPayInput, &$out_trade_no, &$paying)
    {
        $paying = 0;
        //①、提交被扫支付
        ...
        if(!array_key_exists("return_code", $result)
            || !array_key_exists("nonce_str", $result)
            || !array_key_exists("result_code", $result))
        {
            ...
            throw new WxPayException("接口调用失败,请确认是否输入是否有误！err_code:".$result['err_code']."out_trade_no:".$result['out_trade_no']."result_code:".$result['result_code']."input".$microPayInput->GetOut_trade_no());
        }
...
//         }
     ...
     * @return 返回查询接口的结果
     */
    public function payStep2($microPayInput, &$queryResult)
    {
        //签名验证
        $out_trade_no = $microPayInput->GetOut_trade_no();
        //③、确认支付是否成功
        ...
        while($queryTimes > 0)
        {
            $queryTimes--;
            $succResult = 0;
            $this->query($out_trade_no, $succResult, $queryResult);
            if($succResult == 1){//支付成功
                return $succResult;
            }else if($succResult == 2){//用户支付中
                return $succResult;
            }else if($succResult == 3){//已撤销
                return $succResult;
            }
        }
        return -1;
     ...
     * @return 返回查询接口的结果
     */
    public function payStep3($microPayInput, &$result)
    {
        //签名验证
        $out_trade_no = $microPayInput->GetOut_trade_no();
        
        //④、次确认失败，则撤销订单
        if(!$this->cancel($out_trade_no, 0, $result))
        {
            throw new WxpayException("撤销单失败！");
        }
...
// 				return false;
	 ...
	 */
	public function query($out_trade_no, &$succCode , &$result = '')
	{
	 ...
	 * @param 调用深度 $depth
	 */
	public function cancel($out_trade_no, $depth = 0, &$result='')
	{
		if($depth > 10){
			return false;
		}
